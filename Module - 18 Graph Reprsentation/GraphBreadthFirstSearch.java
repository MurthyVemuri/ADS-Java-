...
public class GraphBreadthFirstSearch {
	public static void main(String[] args) {
		...
		List<Integer> arrayNumbersCase = new ArrayList<Integer>();
		...
		if(choice == 1) {
			...
			String outputValue;
			for(int i = 0; i < (verticesNumber * 2) + 1; i++) {
				outputValue= scan.next();
				String[] eachValue = outputValue.split("->");
				for(int j = 0; j < eachValue.length; j++) {
					hs.add(Integer.parseInt(eachValue[j]));
				}
			}
			arrayNumbersCase.addAll(hs);
			for(int i = 0; i < arrayNumbersCase.size() ; i++){ 
				boolean checking = false; 
				for(int j = 0 ; j < i ; j++){ 
					if(arrayNumbersCase.get(i) == arrayNumbersCase.get(j)){ 
						checking = true; 
						break; 
					} 
				} 
				if(!checking){ 
				System.out.print(arrayNumbersCase.get(i) + " "); 
				} 
			}
		}
	}
